...
class PlatoonAccelEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # Create spaces
        high_a = np.array([self.MAX_ACC, self.MAX_ANG_ACC, self.MAX_ACC, self.MAX_ANG_ACC])
        high_s = np.array([np.sqrt(2.0)*self.MAX_POS, 1.0, 1.0, self.MAX_VEL, self.MAX_ANG_VEL, np.sqrt(2.0)*self.MAX_POS, 1.0, 1.0, self.MAX_VEL, self.MAX_ANG_VEL])
        low_s = - high_s
        low_s[0] = 0.0
        low_s[5] = 0.0
    ...
    def reset(self):
        self.state = self.np_random.uniform(low=-0.2, high=0.2, size=(10, ))
        self.state[0] -= self.MAX_POS
        self.state[5] -= self.MAX_POS
    ...
    def step(self, action):
        ...
        ns[5:7] = np.clip(ns[5:7], -self.MAX_POS, self.MAX_POS)
        ns[7] = self._angle_normalize(ns[7])

        ns[3] = np.clip(ns[3], -self.MAX_VEL, self.MAX_VEL)
        ns[4] = np.clip(ns[4], -self.MAX_ANG_VEL, self.MAX_ANG_VEL)
        ns[8] = np.clip(ns[8], -self.MAX_VEL, self.MAX_VEL)
        ns[9] = np.clip(ns[9], -self.MAX_ANG_VEL, self.MAX_ANG_VEL)
        ...
        punish += 0.5 * (obs[5] > self.AGENT_DISTANCE) + 0.5 * (obs[6] < 0.0)
    ...
    def _get_obs(self):
        ...
        rth = self._angle_normalize( np.arctan2(s[1] - s[6], s[0] - s[5]) - s[7] )
        rtv[5] = np.linalg.norm(s[0:2] - s[5:7])
        rtv[6] = np.cos(rth)
        rtv[7] = np.sin(rth)
        rtv[8] = s[8]
        rtv[9] = s[9]
    ...
    def _dynamics(self, s, a, dt):
        ...
        return s + np.array([dx1, dy1, dth1, ddv1, ddw1, dx2, dy2, dth2, ddv2, ddw2])
    ...
    def render(self, mode='human'):
        ...
        jtransform = rendering.Transform(rotation=s[7], translation=[s[5], s[6]])
